...
def detectShape( img, frameNumber, totalFrames ):
    # global variables
    monitor = np.zeros([800, 600, 3], dtype='uint8')                                        # <- Window where game progress is drawn
    ...
    winnerLineOffset = [-200, 0, 200]                                                       # <- used to "move" the line that indicates the winner to proper position
    shape = "NA"                                                                            # initialize the shape as NA - not available
    gameStatus = 0          # initialize game as tie
    ...
    cv.imshow('Thresholded static video.', thresh_img)
    ...
    for c in range(len(contour_list)):
        ...
        if (M["m00"] != 0):
            ...
            if ((len(approx) < 4) or (len(approx) > 9)):        # rectangle or border detected -> skip
                ...
                if(abs(approx[0][0][0]-approx[3][0][0]) < 100 and abs(approx[0][0][0]-approx[3][0][0]) > 20):       # center field of grid detected -> use for individual field detection
                    #print("here")
                    TopLeftPoint = approx[0][0]
                    BotLeftPoint= approx[1][0]
                    BotRightPoint = approx[2][0]
...
def detectGrid( img ):
    ...
    edges_blurred = cv.GaussianBlur(edges, (5, 5), 3)                                               # add blur
    lines = cv.HoughLinesP( edges_blurred, 1, np.pi / 180, 150, minLineLength=70, maxLineGap=5 )    # detect lines using probabilistic method
    print("Number of detected lines: " + str(len(lines)))
    cv.imshow('Detected edges with added blur.', edges_blurred)
...
def frameIsStatic( curr_img ): #, prev_img ):
    ...
    cv.imshow('Binarized video.', curr_img_bin)
    if ((sum > 6000)):
        return False
    else:
